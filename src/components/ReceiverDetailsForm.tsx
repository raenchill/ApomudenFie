...
const ReceiverDetailsForm: React.FC<ReceiverDetailsFormProps> = ({ onSubmit }) => {
  ...
  return (
    <form className="max-w-lg mx-auto p-8 min-h-[500px] flex flex-col items-center justify-center bg-gradient-to-br from-green-50 via-white to-blue-100 rounded-3xl shadow-2xl border border-blue-100" onSubmit={handleSubmit} style={{ perspective: 1000 }}>
      <h2 className="text-3xl font-extrabold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-green-700 via-blue-500 to-teal-400 drop-shadow-lg" style={{ letterSpacing: 1 }}>Enter Your Delivery Details</h2>
      ...
      <div className="w-full space-y-6">
        ...
        <div className="relative">
          ...
          <label className={`absolute left-5 top-4 text-lg transition-all duration-200 pointer-events-none peer-placeholder-shown:top-4 peer-placeholder-shown:text-lg peer-focus:-top-5 peer-focus:text-sm bg-white/80 px-2 rounded-xl shadow-sm ${
            errors.name ? 'text-red-700 peer-focus:text-red-700' : 'text-gray-500 peer-focus:text-green-700'
          }`}>
            <User className="inline w-5 h-5 mr-1 text-green-400" /> Full Name
          </label>
          {errors.name && (
        ...
        <div className="relative">
          <input
            ...
            onChange={handleChange}
            className={`peer w-full bg-white/60 backdrop-blur-lg border-2 rounded-xl px-5 py-4 text-lg shadow-lg focus:outline-none focus:ring-2 transition-all duration-200 placeholder-transparent ${
              errors.address ? 'border-red-300 focus:ring-red-400 focus:border-red-400' : 'border-blue-200 focus:ring-blue-400 focus:border-green-400'
            }`}
            placeholder="Address"
            required
          />
        ...
        <div className="relative">
          <input
            ...
            onChange={handleChange}
            className={`peer w-full bg-white/60 backdrop-blur-lg border-2 rounded-xl px-5 py-4 text-lg shadow-lg focus:outline-none focus:ring-2 transition-all duration-200 placeholder-transparent ${
              errors.phone ? 'border-red-300 focus:ring-red-400 focus:border-red-400' : 'border-green-200 focus:ring-green-400 focus:border-blue-400'
            }`}
            placeholder="Phone Number (e.g., +233 50 123 4567)"
            required
          />
          <label className={`absolute left-5 top-4 text-lg transition-all duration-200 pointer-events-none peer-placeholder-shown:top-4 peer-placeholder-shown:text-lg peer-focus:-top-5 peer-focus:text-sm bg-white/80 px-2 rounded-xl shadow-sm ${
            errors.phone ? 'text-red-700 peer-focus:text-red-700' : 'text-gray-500 peer-focus:text-green-700'
          }`}>
            <Phone className="inline w-5 h-5 mr-1 text-green-400" /> Phone Number
          </label>
          {errors.phone && (
        ...
        <div className="relative">
          <input
            ...
            onChange={handleChange}
            className={`peer w-full bg-white/60 backdrop-blur-lg border-2 rounded-xl px-5 py-4 text-lg shadow-lg focus:outline-none focus:ring-2 transition-all duration-200 placeholder-transparent ${
              errors.email ? 'border-red-300 focus:ring-red-400 focus:border-red-400' : 'border-blue-200 focus:ring-blue-400 focus:border-green-400'
            }`}
