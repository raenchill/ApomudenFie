...
const DeliveryProgress: React.FC<DeliveryProgressProps> = ({ rider, receiver, cartItems, user }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 via-white to-violet-100 rounded-3xl shadow-2xl" style={{ perspective: 1200 }}>
      <h2 className="text-3xl font-extrabold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-blue-700 via-violet-500 to-indigo-400 drop-shadow-lg" style={{ letterSpacing: 1 }}>Delivery Progress</h2>
      {/* 3D Cards */}
      <div className="flex flex-col md:flex-row gap-8 w-full mb-10 justify-center items-center">
        ...
        <div className="relative bg-white/60 backdrop-blur-lg rounded-2xl shadow-2xl p-6 flex flex-col items-center transform hover:scale-105 transition-transform duration-300 border border-blue-100" style={{ boxShadow: '0 8px 32px 0 rgba(31, 38, 135, 0.25)', transform: 'rotateY(-8deg)' }}>
          <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-blue-400 via-violet-300 to-indigo-200 shadow-lg flex items-center justify-center mb-3 border-4 border-white/80">
          ...
          <div className="text-sm text-gray-600 flex items-center gap-1"><Phone className="w-4 h-4 text-violet-500" /> {rider.phone}</div>
          <div className="text-xs text-gray-500 mt-1">ID: {rider.id}</div>
          <div className="text-xs text-violet-700 font-semibold bg-violet-100 rounded-full px-3 py-1 mt-2 shadow">Vehicle: {rider.vehicleNumber}</div>
        </div>
        {/* Receiver Card */}
        <div className="relative bg-white/60 backdrop-blur-lg rounded-2xl shadow-2xl p-6 flex flex-col items-center transform hover:scale-105 transition-transform duration-300 border border-violet-100" style={{ boxShadow: '0 8px 32px 0 rgba(31, 38, 135, 0.18)', transform: 'rotateY(8deg)' }}>
          <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-violet-400 via-blue-200 to-yellow-100 shadow-lg flex items-center justify-center mb-3 border-4 border-white/80">
            <User className="w-16 h-16 text-violet-600" />
          </div>
          <div className="font-bold text-lg text-violet-800 flex items-center gap-2"><User className="w-5 h-5 text-violet-400" /> {receiver.name}</div>
      ...
      <div className="relative w-full max-w-xl mb-10">
        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-8 bg-gradient-to-r from-blue-200 via-violet-100 to-indigo-100 rounded-full shadow-inner border border-blue-100" style={{ zIndex: 1 }} />
        <div className="absolute left-0 top-1/2 -translate-y-1/2 h-8 rounded-full bg-gradient-to-r from-blue-500 via-violet-400 to-indigo-300 shadow-lg transition-all duration-700" style={{ width: `${progressPercent}%`, zIndex: 2 }} />
        {/* 3D Delivery Icon */}
      ...
      <div className="flex gap-4 mt-4">
        <button
          className="bg-gradient-to-r from-blue-600 to-violet-500 text-white px-6 py-3 rounded-2xl shadow-xl font-bold text-lg hover:scale-105 transition-transform duration-200 disabled:opacity-50"
          onClick={handleNext}
        ...
        <button
          className="bg-gradient-to-r from-violet-600 to-blue-500 text-white px-6 py-3 rounded-2xl shadow-xl font-bold text-lg hover:scale-105 transition-transform duration-200 disabled:opacity-50"
      ...
      {currentStep === steps.length - 1 && (
        <div className="mt-8 flex flex-col items-center gap-6">
          <div className="text-center text-violet-700 font-extrabold text-2xl animate-pulse drop-shadow-lg">Delivery Completed!</div>
          ...
          {hasRated && (
            <div className="text-center bg-violet-50 p-4 rounded-xl border border-violet-200">
              <div className="flex items-center justify-center gap-2 mb-2">
                <Star className="w-5 h-5 text-violet-600" />
                <span className="text-violet-700 font-semibold">Thank you for rating!</span>
              </div>
              <p className="text-violet-600 text-sm">Your feedback helps improve our service.</p>
            </div>
          )}
          
          <button
            className="flex items-center gap-2 bg-gradient-to-r from-blue-700 to-violet-500 text-white px-8 py-4 rounded-2xl shadow-xl font-bold text-lg hover:scale-105 transition-transform duration-200"
